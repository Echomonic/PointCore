...
public class CommandPoints implements CommandExecutor, TabCompleter {
                    ...
                    try{
                        ...
                        switch (args[0].toLowerCase()) {
                            ...
                            case "set":
                                ...
                                PointCore.getInstance().getFileManager().setPoints(target.getUniqueId(), points);
                                ...
                                target.sendMessage(ChatColor.YELLOW  + "Your points have been set to " + ChatColor.AQUA + ChatColor.BOLD +
                                        PointCore.getInstance().getFileManager().getPoints(target));
                                spawnFirework(target.getLocation());
                                break;
                            case "add":
                                assert target != null;
                                PointCore.getInstance().getFileManager().setPoints(target.getUniqueId(),
                                        PointCore.getInstance().getFileManager().getPoints(target) + points);
                                ...
                                target.sendMessage(ChatColor.YELLOW  + "You now have " + ChatColor.AQUA + ChatColor.BOLD +
                                        PointCore.getInstance().getFileManager().getPoints(target) + ChatColor.RESET + ChatColor.YELLOW + " points");
                                spawnFirework(target.getLocation());
                                break;
                        }
    ...
    public void spawnFirework(Location location) {
        ...
        }.runTaskLater(PointCore.getInstance(), 4-5);
    }
